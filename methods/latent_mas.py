...
# 古いリスト形式のキャッシュ（タプル）と、最新の DynamicCache オブジェクトの間の仕様の違いを吸収する
from transformers.cache_utils import DynamicCache

def cache_getitem(self, idx):
    if hasattr(self, "key_cache"):
        return (self.key_cache[idx], self.value_cache[idx])
    else:
        return tuple(self)[idx]

def cache_setitem(self, idx, value):
    k_list = getattr(self, "key_cache", getattr(self, "_key_cache", []))
    v_list = getattr(self, "value_cache", getattr(self, "_value_cache", []))
    if idx < len(k_list):
        k_list[idx] = value
        v_list[idx] = value
    else:
...
DynamicCache.__getitem__ = cache_getitem
DynamicCache.__setitem__ = cache_setitem

# transformers が自動で付与する cache_position という指示を、Qwen3モデルが受け取れずにエラーになるのを防ぐ
from transformers.generation.utils import GenerationMixin

original_validate = GenerationMixin._validate_model_kwargs

def patched_validate(self, model_kwargs):
    # エラーの原因となる 'cache_position' をこっそり削除する
